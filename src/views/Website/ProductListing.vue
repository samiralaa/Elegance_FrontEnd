...
<script setup>
  ...
  // Helpers
  const getImageUrl = (path) => `https://backendtest.test/storage/${path}`

  // API Actions
  const addToFavorites = async (product) => {
    try {
      const response = await axios.post(
        'https://backendtest.test/api/favorites',
  ...
  const fetchCategories = async () => {
    try {
      const res = await axios.get('https://backendtest.test/api/website')
  ...
  const fetchProducts = async () => {
    try {
      const res = await axios.get('https://backendtest.test/api/website/products/section')
